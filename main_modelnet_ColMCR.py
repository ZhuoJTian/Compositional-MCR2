...
from general_utils import plot_corZ, visualize_samples
...
def set_up_clients(args, adj, models):
    datasets_train, datasets_test = ModelNet10.create_datasets(args.num_clients)
    ...
    for client_id in range(args.num_clients):
        ...
        path_model = "./ModelNet/Exp_results/ColMCR2/"+"models/"+ str(client_id)
        checkpoint = torch.load(path_model + "_netD.pt")
        client.netD = copy.deepcopy(encoder).to("cpu")
        client.netD.load_state_dict(checkpoint['model_state_dict'])
        client.netD.eval()
        opt_D = torch.optim.Adam(client.netD.parameters(), lr=0.001, weight_decay=args.weight_decay)
        ...
        opt_C = torch.optim.Adam(client.netC.parameters(), lr=0.001, weight_decay=args.weight_decay)
...
def main():
    ...
    args.num_classes = 10
    args.path_result = "./ModelNet/Exp_results/ColMCR4/"
    args.epochs = 8000
    models = ['simplecnn' for i in range(args.num_clients)]
    ...
    for client in clients:
        dataset = client.dataset_train
        visualize_samples(dataset, "./ModelNet/datasamples_"+f"{client.id}.png", num_samples=8)
    '''
